...
from airdot.helpers.url_helper import Url
...
from airdot.helpers.model_helpers import SklearnPredictor
...
class airdotDeployer:
    ...
    def build_deployment(
        ...
        sklearn_predictor = SklearnPredictor(skpredictor=func)
        sklearn_predictor.makeDeployment()
        if callable(func):
        ...
        return {
            "sourceFile": source_file.asDict(),
            "valueFiles": {},
            "name": func_props.name,
            "dataFiles": data_files,
            "module": name,
            "argNames": func_props.argNames,
            "argTypes": func_props.argTypes,
            "requirementsTxt": "\n".join(env_python_packages),
            "pythonVersion": python_version,
            "systemPackages": None,  # need to see this
    ...
    def _build_url(self, json_string, deploy_dict):
        ...
        data_dict = {}
        if len(deploy_dict["argNames"]) > 0:
            for arg_name in deploy_dict["argNames"]:
